...
import { useAuth } from "../app/contexts/AuthProvider";   // <-- key change
...
export default function NavBar() {
  ...
  return (
    <View style={{
      height: 56, paddingHorizontal: 16, alignItems: "center",
      flexDirection: "row", justifyContent: "space-between",
      borderBottomWidth: 1, borderBottomColor: "#E5E7EB", backgroundColor: "#fff"
    }}>
      <Link href="/home" asChild>
        <Pressable><Text style={{ fontSize: 16, fontWeight: "700" }}>Home</Text></Pressable>
      </Link>
      ...
      {user ? (
        ...
        <View style={{ flexDirection: "row", gap: 12 }}>
          <Link href="/(auth)/sign-in" asChild>
            <Pressable style={{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, backgroundColor: "#F3F4F6" }}>
              <Text style={{ fontWeight: "600" }}>Sign in</Text>
            </Pressable>
          </Link>
          <Link href="/(auth)/sign-up" asChild>
            <Pressable style={{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, backgroundColor: "#111827" }}>
